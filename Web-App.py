...
def main():
    ...
    if language == "English":
        if select_page == "CONTACT":
            ...
            if recipe_button:
                if choice_way in ["Upload an image", "Choose from avaialble images"]:
                ...
                # elif choice_way == "Upload a video":
                    # detect(weights="yolov5s_people.pt", source=tfile.name)

                elif choice_way == "Use the webcam":
                    stframe = st.empty()
                    stbutton = st.empty()

                    a = stbutton.button("Stop")
